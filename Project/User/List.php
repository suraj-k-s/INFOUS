...
<html lang="en">
  ...
  <body>
    <h1 align="center" id="heading">Time Chart</h1>
    <table
      id="example"
    ...
    >
      ...
      <tbody>
        <?php                   
                ...
                $i = 0;
                    $selQry = "SELECT s.schedule_id as sid, r1.route_name as route,s.schedule_time as stime,s.bus_name as bus,l1.place_name as froml,l2.place_name as tol,s1.stop_time as st1,s2.stop_time as st2,s1.stop_km as skm1,s2.stop_km as skm2 FROM tbl_schedule s inner join  tbl_route r1 on s.route_id=r1.route_id inner JOIN tbl_stop s1 ON (s1.route_id = r1.route_id) inner JOIN tbl_place l1 ON (s1.place_id = l1.place_id) inner JOIN tbl_stop s2 ON (s2.route_id = r1.route_id) inner JOIN tbl_place l2 ON (s2.place_id = l2.place_id) where l1.place_id  = '" .$fromPlace. "' and l2.place_id = '" .$toPlace. "' and s1.stop_number < s2.stop_number";
					...
					$sTime = new DateTime($time);
					$sTime ->add(new DateInterval('PT' . $st1 . 'M'));
					$ftime = $sTime->format('h:i A');
					
					
					$rTime = new DateTime($time);
					$rTime ->add(new DateInterval('PT' . $st2 . 'M'));
					$ttime = $rTime->format('h:i A');
					
					
					$km =  $data["skm2"] - $data["skm1"];
        ...
        <tr>
          ...
          <td><?php echo $data["froml"];?></td>
          <td><?php echo $ftime;?></td>
          <td><?php echo $data["tol"];?></td>
          <td><?php echo $ttime;?></td>
          <td><?php echo $km;?></td>
          <td><a href="Slot.php?id=<?php echo $data["sid"];?>&amount=<?php echo $amount;?>">Book Now</a></td>
        </tr>
        <?php
                    }
          ...
          buttons: [
            {
              ...
              pageSize: "A5", //A3 , A4 , A5 , A6 , legal , letter
              exportOptions: {
                columns: ":visible",
                search: "applied",
                order: "applied",
